...
> ⚙️ 进阶 → 🔬 深入 | 前置知识：[文生图技术栈](./01-text-to-image.md)，了解 Vision Transformer (ViT) 基础，[Score Matching](../01-fundamentals/06-score-matching.md)
...
**Rectified Flow（整流流）训练**：SD3 不使用 DDPM 的噪声预测，而是使用 Flow Matching 框架（参见 [Flow Matching](../02-models-zoo/06-flow-matching.md)）。具体地，SD3 使用 Rectified Flow——学习从噪声到数据的直线路径：
...
> **下一篇**：[文生视频技术](./03-text-to-video.md) — 从单帧到时间序列，扩散模型如何处理时间维度。
