...
> 🔰 入门 | 前置知识：[扩散模型核心直觉](../01-fundamentals/01-core-intuition.md)
...
**跨领域应用注意**：条件生成不仅用于图像——在 VLA（视觉-语言-动作模型）领域，扩散/Flow Matching 模型以视觉和语言为条件生成机器人动作序列。参见 [VLA: Diffusion 作为动作生成器](../../../vla/07-foundation-models/03-diffusion-action.md)。
...
| 文档 | 内容 | 核心问题 | 难度 |
|------|------|---------|------|
| [Classifier Guidance](./02-classifier-guidance.md) | 分类器梯度引导 | 如何用外部分类器控制生成 | ⚙️ |
| [CFG](./03-cfg.md) | 无分类器引导 | 如何不需要额外分类器就能引导 | ⚙️ |
| [ControlNet](./04-controlnet.md) | 结构控制网络 | 如何精确控制空间结构 | ⚙️ |
| [IP-Adapter](./05-ip-adapter.md) | 图像提示适配 | 如何用参考图像控制风格 | ⚙️ |
| [适配器与微调](./06-adapters.md) | LoRA、DreamBooth 等 | 如何让模型学习新概念 | ⚙️ |
| [方法对比](./07-comparison.md) | 综合对比表 | 如何选择合适的方法 | 🔰 |

---

> **下一篇**：[Classifier Guidance](./02-classifier-guidance.md)
