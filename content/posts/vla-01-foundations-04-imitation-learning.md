...
> 🔰 入门 → ⚙️ 进阶 | 前置知识：[观测空间与动作空间](./03-observation-action-space.md)
> 交叉引用：[强化学习基础](./05-reinforcement-learning.md)、[Diffusion 动作生成](../07-foundation-models/03-diffusion-action.md)、[VLA 未来展望](../07-foundation-models/06-future.md)
...
VLA 训练全流程分三阶段：**阶段 1**——VLM 互联网预训练（LAION-5B/WebLI，图文对比+VQA，获得通用视觉-语言表征）；**阶段 2**——机器人数据 BC 微调（Open X-Embodiment 数据，观测→动作映射，损失为 MSE/CE/Flow Matching）；**阶段 3**——可选的 RL 对齐微调（RLHF/DPO，人类偏好标注，详见[强化学习基础](./05-reinforcement-learning.md)）。
...
> **下一篇**：[强化学习基础](./05-reinforcement-learning.md)
