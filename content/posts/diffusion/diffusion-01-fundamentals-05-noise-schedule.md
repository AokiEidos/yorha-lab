...
> ⚙️ 进阶 | 前置知识：[前向过程详解](./02-forward-process.md)，了解 $\bar{\alpha}_t$ 和 SNR 的定义
...
在 SDE 框架（详见 [SDE/ODE 统一视角](./07-sde-ode.md)）中，噪声调度被推广到连续时间 $t \in [0, 1]$：
...
> **下一篇**：[得分函数与得分匹配](./06-score-matching.md) — 从另一个视角理解扩散模型：得分函数和去噪得分匹配。
