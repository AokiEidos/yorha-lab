...
> ⚙️ 进阶 | 前置知识：[Latent Diffusion 与 Stable Diffusion](../02-models-zoo/04-latent-diffusion.md)，[CFG](../03-conditional-generation/03-cfg.md)，[Flow Matching](../02-models-zoo/06-flow-matching.md)
...
核心原因是 **Scaling Laws（缩放定律）**。LLM 领域的核心发现是"模型越大、数据越多、效果越好"的幂律关系。U-Net 由于其多尺度编码器-解码器的固有结构，参数规模难以持续扩展（超过 3B 后架构设计变得困难）。而 DiT 使用标准 Transformer，可以像 LLM 一样简单地堆叠层数来扩大规模，天然适配 Scaling Laws。详见 [DiT 架构深度解析](./02-dit.md)。

---

## 三、图像解码器（VAE）

**变分自编码器（Variational Autoencoder, VAE）** 负责像素空间和潜空间之间的转换。详细原理参见 [Latent Diffusion](../02-models-zoo/04-latent-diffusion.md)。
...
从 DDPM 到 Flow Matching 的迁移带来了显著的采样效率提升——SD3 在 28 步采样下即可达到 SD 1.5 在 50 步下的质量，详见 [Flow Matching](../02-models-zoo/06-flow-matching.md)。
...
> **下一篇**：[DiT 架构深度解析](./02-dit.md) — 深入 Transformer 如何取代 U-Net 成为扩散模型的新骨干。
