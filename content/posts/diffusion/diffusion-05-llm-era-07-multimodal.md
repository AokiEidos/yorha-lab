...
这与自动驾驶中的**世界模型（World Model）** 理念相通（参见 [世界模型](../vla-06-autonomous-driving-06-world-model.md)），也是 VLA（Vision-Language-Action）架构的核心思想——Agent 通过 Diffusion World Model 预测动作后果，选择最优策略。
...
> **下一篇**：[自动驾驶与 Diffusion 全景](../vla-06-autonomous-driving-01-overview.md) — 进入模块六，Diffusion 在自动驾驶中的应用。
